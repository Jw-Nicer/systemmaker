...
### Phase 7 Fixes — Chat Agent Quality Pass (2026-04-10 / 2026-04-11)

| ID | Issue | Root Cause | Fix | Files Changed |
|----|-------|-----------|-----|---------------|
| BUG-028 | Chat agent re-asks for email after the visitor submits the in-bubble form | `EmailCaptureInline` was unmounted when `emailForm.status === "sent"` (parent dropped the prop), so its built-in success branch was unreachable. Also, captured contact info wasn't synced into `extracted` so the next follow-up turn's prompt didn't know about it. | Removed the `status !== "sent"` guard so `EmailCaptureInline` stays mounted and renders its success state ("Plan sent! Check your inbox." + booking CTA). New `markEmailCaptured(name, email)` action on `useSSEChat` reducer flips `email_auto_sent: true` and merges name/email into `extracted`. Updated `buildFollowUpPrompt` to surface name/email in "Their situation" and replace instruction #5 with an explicit "do NOT re-ask" rule when email is on file. | `components/marketing/AgentChat.tsx`, `components/marketing/ChatMessages.tsx`, `hooks/useSSEChat.ts`, `lib/agents/conversation.ts`, `tests/sse-chat-reducer.test.ts` |
| BUG-029 | Plan section cards in chat displayed raw JSON instead of formatted summaries | The chat route emitted `content: JSON.stringify(data)` for each `plan_section` event, and `ChatPlanCard` rendered it via `whitespace-pre-wrap`, dumping `{"stages":[{"name":"Intake",...}]}` straight into the bubble | Rewrote `ChatPlanCard` to parse the JSON per `sectionType` and render compact per-section bodies (`IntakeBody`, `WorkflowBody`, `AutomationBody`, `DashboardBody`, `OpsPulseBody`, `RoadmapBody`). Threaded `sectionType` through `MessageBubble`. Hid the raw JSON `content` from the bubble body in `AgentChat`'s display-message mapping. | `components/marketing/ChatPlanCard.tsx`, `components/marketing/ChatMessages.tsx`, `components/marketing/AgentChat.tsx` |
| BUG-030 | `inferBottleneck` over-fired on long messages with one keyword in passing (e.g., "I'd love to learn how the manual review process works for new client onboarding" — has `manual`, length > 50, no actual problem statement) | Conversational branch fired on `(pattern OR (length>50 AND keyword))` — the length-based fallback was too lax | Tightened to `(pattern AND keyword)`. Both signals now required. The explicit `bottleneck is X` first branch is unchanged. | `lib/agents/conversation.ts`, `tests/conversation-phases.test.ts` |
| BUG-031 | `inferIndustry` missed conversational openers like "we're a 30-person property management shop" | The original "I run a..." pattern didn't catch sized/scoped opener forms; the bare-name pattern required the message to BE the industry name | Added Branch 2 to `inferIndustry` matching `we're/we are/I'm/I am + optional sizing prefix + industry name + business suffix (shop/company/firm/...)`. Same negative-words guard prevents false captures from "we're using HubSpot". | `lib/agents/conversation.ts`, `tests/conversation-phases.test.ts` |
| BUG-032 | Generic safety fallback fired mid-conversation on transient Gemini errors, starting the conversation over | `generateConversationalResponse` had no retry logic and yielded `buildSafeConversationFallback` (which says "I can't access your systems...") on the first stream error, regardless of whether content had already been sent | Added single-retry wrapper that only retries when no chunks have been yielded yet (you can't un-yield text) and only on transient errors. Replaced the safety fallback with a new contextual fallback (`buildContextualConversationFallback`) that re-asks the next missing field in gathering, asks about the summary in confirming, asks the user to repeat in follow_up — never starts over. Mid-stream failures stop cleanly without yielding fallback. | `lib/agents/conversation.ts`, `tests/conversation.test.ts` |
| BUG-033 | `is_extraction_update` SSE event echoed full extracted state every gathering turn even when nothing changed | `route.ts` emitted the update unconditionally based on phase, ignoring whether extraction actually produced new fields. Wasted bandwidth and forced an unnecessary client re-render every turn | Added `extractedHasChanges(before, after)` helper that compares all 7 ExtractedIntake fields treating undefined and "" as equivalent. Route now only emits the event when `phaseChanged \|\| dataChanged`. | `lib/agents/conversation.ts`, `app/api/agent/chat/route.ts`, `tests/conversation.test.ts` |
| BUG-034 | Lead-scoring `case "critical"` was dead code — no form schema (`leadSchema`, `agentChatSchema`, `guidedAuditSchema`) ever emitted that value, but the test suite pinned it as a load-bearing branch. Originally documented as a live bug in `GAPS_SHARED_CONTEXT.md` (D9), but had been partially fixed at some point (a `case "urgent":` was added next to `case "critical":`) leaving stale dead code. | At one moment the bug existed and was partially fixed without removing the dead branch or updating the test | Replaced the switch statement with a `URGENCY_SCORES` lookup table tied to the `z.enum` declarations in `lib/validation.ts` via comment. Updated tests: renamed "urgency critical adds 20" → "urgency urgent adds 20", added a regression-net test that pins `urgency: "critical"` → 0 (legacy dead value), added a structural test that iterates over the canonical `{low, medium, high, urgent}` set and verifies each scores correctly. | `lib/leads/scoring.ts`, `tests/lead-scoring.test.ts` |
| BUG-035 | Hardcoded `INDUSTRY_PROBING` table covered only 8 verticals — adding a new industry required a code change + redeploy | Domain knowledge baked into source code that should have been admin-configurable | New `industry_probing` Firestore collection with admin CRUD page at `/admin/industry-probing`, server-side reader with TTL cache + alias map + graceful Firestore-failure fallback, server actions with cache invalidation, idempotent seed script for the 8 defaults. Hardcoded table is now `INDUSTRY_PROBING_FALLBACK` — only used as a safety net when Firestore is empty/unreachable. | `types/industry-probing.ts`, `lib/firestore/industry-probing.ts`, `lib/actions/industry-probing.ts`, `app/admin/(authenticated)/industry-probing/`, `lib/agents/conversation.ts`, `app/api/agent/chat/route.ts`, `firestore.rules`, `scripts/seed-industry-probing.ts`, `lib/admin/sidebar-config.ts`, `package.json` |
