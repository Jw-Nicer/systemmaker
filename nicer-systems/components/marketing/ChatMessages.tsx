  ...
  if (status === "sent") {
    return (
      <div className="mt-3 space-y-3">
        <div className="rounded-[var(--radius-sm)] border border-green-500/20 bg-green-500/10 p-3 text-center">
          <p className="text-sm font-medium text-green-700">Plan sent! Check your inbox.</p>
        </div>
        <button
          onClick={() => {
            track(EVENTS.BOOKING_CLICK, { source: "chat_email_sent" });
            setShowBooking(true);
          }}
          className="flex w-full items-center justify-center gap-2 rounded-full bg-[#171d13] px-5 py-2.5 text-sm font-semibold text-[#f7f2e8] transition-transform hover:scale-[1.02]"
        >
